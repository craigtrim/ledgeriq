#!/usr/bin/env bash
#──────────────────────────────────────────────────────────────────────────────
# 007 - Create Textract VPC Endpoint
#──────────────────────────────────────────────────────────────────────────────
# Description:
#   Creates a Textract VPC Endpoint to allow Lambda functions in the VPC to
#   access AWS Textract without requiring NAT Gateway or internet access.
#
# Purpose:
#   VPC Lambdas lose internet access by default. This endpoint provides a
#   private route to Textract, avoiding connection timeout errors.
#
# Cost:
#   Interface VPC Endpoints for Textract have hourly charges:
...
#
# Usage:
#   ./007-create-textract-endpoint.sh
#
# Prerequisites:
...
# Textract service name (region-specific)
TEXTRACT_SERVICE_NAME="com.amazonaws.${REGION}.textract"
...
#──────────────────────────────────────────────────────────────────────────────
# Check if Textract Endpoint Already Exists
#──────────────────────────────────────────────────────────────────────────────

log_info "Checking if Textract VPC endpoint already exists..."
...
EXISTING_ENDPOINT=$(aws ec2 describe-vpc-endpoints \
    ...
    --filters \
        ...
        "Name=service-name,Values=${TEXTRACT_SERVICE_NAME}" \
    --query 'VpcEndpoints[0].VpcEndpointId' \
    --output text 2>/dev/null || echo "None")

if [[ -n "$EXISTING_ENDPOINT" ]] && [[ "$EXISTING_ENDPOINT" != "None" ]]; then
    log_warning "Textract VPC endpoint already exists: ${EXISTING_ENDPOINT}"
    log_info "No action needed"
    exit 0
fi

#──────────────────────────────────────────────────────────────────────────────
# Create Textract VPC Endpoint
#──────────────────────────────────────────────────────────────────────────────

log_info "Creating Textract VPC endpoint..."
log_info "Service: ${TEXTRACT_SERVICE_NAME}"
log_info "VPC: ${VPC_ID}"
...
VPC_ENDPOINT_ID=$(aws ec2 create-vpc-endpoint \
    ...
    --service-name "$TEXTRACT_SERVICE_NAME" \
...
if [[ -z "$VPC_ENDPOINT_ID" ]]; then
    log_error "Failed to create Textract VPC endpoint"
    exit 1
fi

log_success "Created Textract VPC endpoint: ${VPC_ENDPOINT_ID}"
...
log_success "Enabled private DNS (allows standard Textract endpoint URLs)"
...
aws ec2 create-tags \
    ...
    --tags \
        Key=Name,Value="${VPC_NAME}-textract-endpoint" \
        Key=ManagedBy,Value=Script \
        Key=Purpose,Value=LambdaTextractAccess
...
echo ""
log_success "Textract VPC Endpoint Setup Complete!"
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  VPC Endpoint ID:    ${VPC_ENDPOINT_ID}"
echo "  Service:            ${TEXTRACT_SERVICE_NAME}"
echo "  VPC:                ${VPC_ID}"
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
log_info "Your Lambda functions can now access Textract privately!"
log_warning "Cost: ~\$7-15/month (Interface Endpoint pricing)"
echo ""
log_warning "Next Steps:"
echo "  1. Wait 1-2 minutes for endpoint to become 'available'"
echo "  2. Re-run your img-to-ocr Lambda (should work now)"
echo "  3. Check CloudWatch logs for successful Textract calls"
echo ""

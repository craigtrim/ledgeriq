...
def generate_cache_key(md5_hash: str, document_hash: str) -> str:
    """Generate S3 cache key following pattern: cache/extract-dates/md5[:2]/md5[2:]/dochash.txt
...
# ═══════════════════════════════════════════════════════════════════════════
# Bedrock Date Extraction
# ═══════════════════════════════════════════════════════════════════════════

def extract_dates(text: str) -> dict:
    """Extract dates using Bedrock Claude 4.5.

    Returns:
        dict with 'dates' (list) and 'error' keys
    """
    ...
    try:
        ...
        return {'dates': None, 'error': f'API error: {error_code}'}
    except Exception as e:
        error_msg = str(e)
        logger.error(
            f"Date extraction failed: {error_msg}", exc_info=True)

        # Check for timeout errors
        if 'timeout' in error_msg.lower() or 'timed out' in error_msg.lower():
            return {'dates': None, 'error': 'Connection timeout'}

        return {'dates': None, 'error': f'Extraction error: {error_msg}'}
...
def handler(event: dict[str, any], _) -> dict:
    ...
    try:
        ...
        if cache_exists:
            ...
            logger.info(f"Cache hit: returning {len(dates)} dates")
            return {
                'statusCode': 200,
                'body': {
                    'results': {
        ...
        # Extract dates using Bedrock
        extraction = extract_dates(document_text)
        ...
        logger.info(
            f"Extraction complete: {len(dates)} dates (cached: {cache_key})")
        ...
        logger.error(
            f"Date extraction handler failed: {str(e)}", exc_info=True)
        return {
            'statusCode': 500,
            'body': {

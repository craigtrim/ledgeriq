...
STATE_MACHINE_ARN = "arn:aws:states:us-west-2:210182908261:stateMachine:extract-line-items"
...
def handler(event: dict, _) -> dict:
    ...
    try:
        ...
        sfn_input_json = dumps(sfn_input)
        logger.info(f"Invoking Step Function: {STATE_MACHINE_ARN}")
        logger.info(f"Step Function input JSON: {sfn_input_json}")
        logger.info(f"Step Function input JSON length: {len(sfn_input_json)}")
